...
# interface on primary.	 The fault causes standby to respond with stale
# flush LSN value, simulating the case that it has not caught up.  If
# the standby is synchronous, commits on primary should wait until
...
sub test_sync_commit
{
	...
	my ($cmdret, $stdout, $stderr) =
	  $primary->psql('postgres', 'create extension faultinjector;', on_error_die => 1);
	
	my $sql = sprintf(
		"select inject_fault_infinite('standby_flush', 'skip', '%s', %d)",
		$standby->host, $standby->port);
	($cmdret, $stdout, $stderr) = $primary->psql('postgres', $sql);
	ok($stdout =~ /Success/, 'inject skip fault in standby');

	# commit a transaction on primary, the primary backend should wait
	# because standby1 hasn't acknowledged the receipt of the commit LSN.
	my $background_psql = startpsql($primary->host, $primary->port);
	...
	($cmdret, $stdout, $stderr) =
	  $primary->psql('postgres', 'checkpoint;', on_error_die => 1);
	($cmdret, $stdout, $stderr) =
	  $primary->psql(
		  'postgres',
		  ...
		  on_error_die => 1);
	ok($stdout =~ /Success/, 'primary WAL has moved ahead of standby');
...
# on primary wait for standby1 to catch up.
test_sync_commit($node_primary, $node_standby_1);
